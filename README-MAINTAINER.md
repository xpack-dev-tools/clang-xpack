...
but only the final patch of each version is released (like 15.0.6).
...
Determine the version (like `15.0.6`) and update the `scripts/VERSION`
file; the format is `15.0.6-1`. The fourth number is the xPack release number
...
and fix them; assign them to a milestone (like `15.0.6-1`).
...
- add a new entry like _* v15.0.6-1 prepared_
- commit with a message like _prepare v15.0.6-1_
...
- fetch `upstream`
- checkout the `llvmorg-15.0.6` tag in detached state HEAD
- create a branch like `v15.0.6-xpack`
...
- add a `v15.0.6-1-xpack` tag; enable push to origin
- select the commit with the patch
- save as patch
- move to `patches`
- rename `llvm-15.0.6.git.patch`
...
total 197704
-rw-r--r--  1 ilg  staff  132413536 Aug 20 13:48 xpack-clang-15.0.6-1-darwin-x64.tar.gz
-rw-r--r--  1 ilg  staff        105 Aug 20 13:48 xpack-clang-15.0.6-1-darwin-x64.tar.gz.sha
```
...
total 165464
-rw-r--r--  1 ilg  staff  110761767 Aug 20 12:48 xpack-clang-15.0.6-1-darwin-arm64.tar.gz
-rw-r--r--  1 ilg  staff        107 Aug 20 12:48 xpack-clang-15.0.6-1-darwin-arm64.tar.gz.sha
```
...
total 196820
-rw-rw-rw- 1 ilg ilg 101828916 Aug 20 13:42 xpack-clang-15.0.6-1-linux-x64.tar.gz
-rw-rw-rw- 1 ilg ilg       104 Aug 20 13:42 xpack-clang-15.0.6-1-linux-x64.tar.gz.sha
```
...
total 41300
-rw-rw-rw- 1 ilg ilg 297910243 Aug 20 15:32 xpack-clang-15.0.6-1-win32-x64.zip
-rw-rw-rw- 1 ilg ilg       101 Aug 20 15:32 xpack-clang-15.0.6-1-win32-x64.zip.sha
```
...
total 169440
-rw-rw-rw- 1 root root 94181557 Aug 21 05:04 xpack-clang-15.0.6-1-linux-arm64.tar.gz
-rw-rw-rw- 1 root root      106 Aug 21 05:04 xpack-clang-15.0.6-1-linux-arm64.tar.gz.sha
```
...
total 154256
-rw-rw-rw- 1 ilg ilg 89795445 Aug 20 20:16 xpack-clang-15.0.6-1-linux-arm.tar.gz
-rw-rw-rw- 1 ilg ilg      104 Aug 20 20:16 xpack-clang-15.0.6-1-linux-arm.tar.gz.sha
```
...
```sh
.../xpack-clang-15.0.6-1/bin/clang --version
xPack x86_64 clang version 15.0.6
```
...
- in `CHANGELOG.md`, add the release date and a message like _* v15.0.6-1 released_
...
tagged like **v15.0.6-1** (mind the dash in the middle!) and
named like **xPack LLVM clang v15.0.6-1** (mind the dash),
...
- commit the `develop` branch of `xpack/web-jekyll` GitHub repo;
  use a message like _xPack LLVM clang v15.0.6-1 released_
- push to GitHub
...
- commit all changes, use a message like
  _package.json: update urls for 15.0.6-1.1 release_ (without _v_)

## Publish on the npmjs.com server

- select the `xpack-develop` branch
- check the latest commits `npm run git-log`
- update `CHANGELOG.md`, add a line like _* v15.0.6-1.1 published on npmjs.com_
- commit with a message like _CHANGELOG: publish npm v15.0.6-1.1_
- `npm pack` and check the content of the archive, which should list
  only the `package.json`, the `README.md`, `LICENSE` and `CHANGELOG.md`;
  possibly adjust `.npmignore`
- `npm version 15.0.6-1.1`; the first 4 numbers are the same as the
...
- `npm dist-tag add @xpack-dev-tools/clang@15.0.6-1.1 latest`
- `npm dist-tag ls @xpack-dev-tools/clang`

In case the previous version is not functional and needs to be unpublished:

- `npm unpublish @xpack-dev-tools/clang@15.0.6-1.1`
...
- paste the release name like **xPack LLVM clang v15.0.6-1 released**
