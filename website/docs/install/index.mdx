...
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fclang?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/clang/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/clang-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fclang?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/clang/)

The **xPack LLVM clang** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
platform specific archives.

## Automated install

The easiest (and recommended) way to install LLVM clang
is via **xpm** and the package available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/clang`}><code>@xpack-dev-tools/clang</code></a> from
...
<code>xpacks/@xpack-dev-tools/clang</code>.
...
**clang** package into the project.

The command to install the **latest** available version of **clang** is:
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/clang@${customField('releaseVersion')} --verbose
`} </CodeBlock>
...
* identify the platform specific archive for the latest available version
  of LLVM clang, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/clang</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/clang/.content/bin</code>;
* add <code>@xpack-dev-tools/clang</code> to
  `package.json` as a
  development dependency; this associates a specific version of LLVM clang with
...
it is also possible to install LLVM clang only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/clang@latest --global --verbose
`} </CodeBlock>
...
In order to access the LLVM clang binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/clang/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/clang/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/clang/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/clang --verbose
`} </CodeBlock>

To completely remove the package from the user global xPacks store:

<CodeBlock language="sh"> {
`xpm uninstall --global @xpack-dev-tools/clang --verbose
`} </CodeBlock>
...
For all platforms, the **xPack LLVM clang** binaries are released as portable
archives that can be installed in any location.

The archives can be downloaded from
the <a href={`https://github.com/xpack-dev-tools/clang-xpack/releases`}>GitHub Releases</a> pages.
...
<ul>
<li><code>xpack-clang-{customField('xpackVersion')}-win32-x64.zip</code></li>
</ul>
...
To manually install the <b>xPack LLVM clang</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\clang</code> folder, for
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\clang\\xpack-clang-{customField('xpackVersion')}</code>.
...
The macOS versions of **xPack LLVM clang**
are packed as `.tar.gz` archives.
Download the latest version named like:

<ul>
<li><code>xpack-clang-{customField('xpackVersion')}-darwin-x64.tar.gz</code></li>
<li><code>xpack-clang-{customField('xpackVersion')}-darwin-arm64.tar.gz</code></li>
</ul>
...
The recommended location is
the <code>~/Library/xPacks/clang</code> folder, for
example <code>/Users/ilg/Library/xPacks/clang/xpack-clang-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/clang
cd ~/Library/xPacks/clang

tar xvf ~/Downloads/xpack-clang-${customField('xpackVersion')}-darwin-x64.tar.gz
chmod -R -w xpack-clang-${customField('xpackVersion')}
`} </CodeBlock>
...
<ul>
<li><code>xpack-clang-{customField('xpackVersion')}-linux-x64.tar.gz</code></li>
<li><code>xpack-clang-{customField('xpackVersion')}-linux-arm64.tar.gz</code></li>
<li><code>xpack-clang-{customField('xpackVersion')}-linux-arm.tar.gz</code></li>
</ul>
...
The recommended location is
the <code>~/.local/xPacks/clang</code> folder, for
example <code>/home/ilg/.local/xPacks/clang/xpack-clang-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/clang
cd ~/.local/xPacks/clang

tar xvf ~/Downloads/xpack-clang-${customField('xpackVersion')}-linux-x64.tar.gz
chmod -R -w xpack-clang-${customField('xpackVersion')}
`} </CodeBlock>
