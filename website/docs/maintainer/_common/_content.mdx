...
- determine the version (like <code>19.1.7-1.1</code>)
...
Determine the version (like <code>19.1.7-1</code>)
and update the `build-assets/scripts/VERSION` file; the format
is <code>19.1.7-1</code>.
...
the **[Update the version specific code](http://localhost:3000/clang-xpack/docs/maintainer/#update-the-version-specific-code)** section.
...
and fix them; assign them to a milestone (like <code>19.1.7-1</code>).
...
- to remove the rest of the dates, change from:
  ...
  *
  ```
- add a new entry like _**\* v19.1.7-1 prepared**_
- **commit** with the message _**prepare v19.1.7-1**_
...
```sh
rm -f ~/Work/xpack-dev-tools/clang-xpack.git/build-assets/package-lock.json
git -C ~/Work/xpack-dev-tools/clang-xpack.git pull
...
(like **19.1.7**) with the following expression:
...
```sh
xpm run trigger-workflow-build-darwin-x64 -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
xpm run trigger-workflow-build-darwin-arm64 -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
xpm run trigger-workflow-build-linux-x64 -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
xpm run trigger-workflow-build-win32-x64 -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
xpm run trigger-workflow-build-linux-arm64 -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
```
...
[GitHub Actions](https://github.com/xpack-dev-tools/clang-xpack/actions) page.
...
```sh
xpm run trigger-travis-macos -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
```
...
tagged like **v19.1.7-1** (mind the dash in the middle!) and
named like **xPack LLVM clang v19.1.7-1** (mind the dash),
...
watching the clang-xpack project.
...
this will add a file in the `website/blog` folder:
  ```sh
  xpm run generate-website-blog-post -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
  ```
...
- add entries in the **Bug fixes**; use the following syntax
  ```
  - [[#N](https://github.com/xpack-dev-tools/clang-xpack/issues/N)]:
  ```
- copy/paste the folders hierarchies from the install guide
- **commit** with the message _**website: blog post release 19.1.7-1 published**_
...
- run the **update-package-binaries** xpm action:
  ```sh
  xpm run update-package-binaries -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
  ```
- open the top `package.json` file
...
- **commit** all changes with the message
  _**package.json: update urls for 19.1.7-1.1 release**_

### Publish on the npmjs.com server

- select the `xpack-development` branch
- check the latest commits `xpm run git-log`
- update `CHANGELOG.md`, add a line like _**\* v19.1.7-1.1 published on npmjs.com**_
- **commit** with the message _**CHANGELOG: publish npm v19.1.7-1.1**_
...
<a href="https://www.npmjs.com/package/@xpack-dev-tools/clang?activeTab=versions">npmjs.com</a>.
...
in https://xpack-dev-tools.github.io/clang-xpack/blog/
...
- paste the release name like _**xPack LLVM clang v19.1.7-1 released**_
...
[![NPM Version](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fclang?color=green)](https://www.npmjs.com/package/@xpack-dev-tools/clang/)
[![NPM Downloads](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fclang)](https://www.npmjs.com/package/@xpack-dev-tools/clang/)
  - all releases [![NPM Downloads](https://img.shields.io/npm/dt/%40xpack-dev-tools%2Fclang)](https://www.npmjs.com/package/@xpack-dev-tools/clang/)
