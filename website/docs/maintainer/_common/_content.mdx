...
**@xpack-dev-tools/clang** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
npm --prefix ~/Work/xpack-dev-tools/clang-xpack.git/build-assets install
xpm install -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
...
(such as **21.1.8-1.1**) with the following expression:
...
To exit it, use `Ctrl-b d`.

For `xbbli`, which has additional memory, and can run multiple jobs in parallel, start two runners:
...
- In `CHANGELOG.md`, add the release date and a message
  such as _**\* v21.1.8-1 released**_
...
tagged such as **v21.1.8-1** (mind the dash in the middle!) and
named such as **xPack LLVM clang v21.1.8-1** (mind the dash),
...
- **Commit** all changes with the message
  _**package.json: update URLs for 21.1.8-1.1 release**_
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/clang-xpack/releases) page
...
npm --prefix ~/Work/xpack-dev-tools/clang-xpack.git/build-assets install && \
xpm install -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets && \
xpm run test-release -C ~/Work/xpack-dev-tools/clang-xpack.git/build-assets
```
