...
named like **xPack LLVM clang v{customField('xpackVersion')}** (mind the dash),
...
npm dist-tag add @xpack-dev-tools/clang@${customField('xpackVersion')}.${customField('npmSubversion')} latest
npm dist-tag ls @xpack-dev-tools/clang
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/clang@${customField('xpackVersion')}.${customField('npmSubversion')}
`} </CodeBlock>
...
- paste the release name like _**xPack LLVM clang v{customField('xpackVersion')} released**_
